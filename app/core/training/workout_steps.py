...
def parse_key_workout_steps(
    ...
    steps: List[Dict[str, Any]] = []
    if has_wcd:
        steps.append(_warmup(pace_zones))
    ...
    if m:
        # Progression long runs are continuous — drop warmup/cooldown
        steps = []
        easy_m = _parse_distance_to_m(m.group(2), "km")
        last_km = m.group(4).split("-")[-1]  # upper bound
    ...
    if m:
        ...
        pace_text = m.group(4)
        zone = _infer_zone(pace_text) or ("T" if workout_type == "tempo" else "I")
        if rep_m >= 1000:
            rep_label = f"{reps} × {rep_m / 1000:.1f} km"
        else:
            rep_label = f"{reps} × {rep_m} m"
        steps.append(
            _step(
                "run",
                rep_label,
