...
# ``tuning.ROAD_LONG_RUN_CAPS`` and must not exceed this by much — when the two
# disagree (a 5K plan prescribing a 14 km long run against an 8 km tier cap) it
# is the tier table that is authoritative for what a plan may prescribe.
...
# big slice) and the ceiling is what a *healthy* plan stays under — a week whose
# long run is 60 %+ of its volume is a plan with one session and some filler.
LONG_RUN_SHARE_LO = 0.20
LONG_RUN_SHARE_HI = 0.35
...
def long_run_share_ceiling(max_runs: int) -> float:
    ...
    The published 35 % band assumes enough runs to spread the week across. At
    2-3 runs we tolerate more (there is nowhere else for the volume to live),
    but never past the point where the week is one run plus filler.
    """
    if max_runs <= 2:
        return 0.60
    if max_runs == 3:
