...
def _is_prescriptive(workout: Dict[str, Any]) -> bool:
...
def _rebuild_long_run(workout: Dict[str, Any],
                      new_distance: float,
                      pace_zones: Optional[Dict]) -> None:
    ...
    rebuilt = workout_builders.generate_long_run(
        workout.get('day', 0), rounded, rounded, pace_zones,
    )
...
def _set_distance(workout: Dict[str, Any], new_distance: float,
                  pace_zones: Optional[Dict] = None) -> None:
    ...
    if workout.get('type') == 'long' and not workout.get('key_workout_id'):
        _rebuild_long_run(workout, rounded, pace_zones)
        return
...
def _scale_down(workouts: List[Dict[str, Any]],
                total_km: float,
                pace_zones: Optional[Dict] = None) -> float:
...
def _fill_shortfall(workouts: List[Dict[str, Any]], total_km: float,
    ...
    ``_rebuild_long_run`` so the workout stays internally consistent.
    """
    if actual_total_km >= total_km * 0.97 or actual_total_km <= 0:
        actual_total_km = round(sum(w.get('distance', 0) for w in workouts), 1)
    else:
        deficit = total_km - actual_total_km
        # Easy and (non-key) long runs both absorb the shortfall, weighted
        # by their current distance. Quality and key-overlay workouts stay
        # at their prescribed dose. ``_set_distance`` rebuilds the long-run
        # description and steps when the long distance changes, so the
        # workout stays internally consistent after expansion.
        flexible = [w for w in workouts
                    if w.get('type') in ('easy', 'long')
                    and not _is_prescriptive(w)
                    and w.get('distance', 0) > 0
        ...
        if flexible:
            ...
            if total_flex > 0:
                for w in flexible:
                    ...
                    _set_distance(w, w['distance'] + share, pace_zones)
        ...
        for w in workouts:
            if w.get('type') == 'easy' and w.get('distance', 0) > long_d:
                if not long_is_prescriptive:
                    # Move headroom from easy → long until easy ≤ long.
                    transferable = w['distance'] - long_d
                    headroom = hard_ceiling - long_d
                    transfer = min(transferable, max(0, headroom))
                    if transfer > 0:
                        _set_distance(w, w['distance'] - transfer, pace_zones)
                        _set_distance(long_w, long_w['distance'] + transfer, pace_zones)
                        long_d = long_w['distance']
                if w['distance'] > long_d + 0.05:
                    # Easy should never exceed the long run; truncate it.
                    # When the long is prescriptive we can't grow it to
                    # absorb the excess — accept the lower weekly total.
                    _set_distance(w, long_d, pace_zones)
