...
class PlanPagesMixin:
    ...
    def _add_title_page(
        ...
        story.append(
            Paragraph(renderer.subtitle_text(training_plan), self.subtitle_style)
        )
    ...
    def _add_weekly_plan(self, story: List, week: Dict[str, Any]):
        if "phase" in week and "phase_description" in week:
            ...
            story.append(
                Paragraph(f"<i>{week['phase_description']}</i>", self.normal_style)
            )
            story.append(Spacer(1, 0.3 * cm))
        else:
            story.append(
                Paragraph(
                    f"Week {week['week']} - {week['total_km']:.1f} km",
                    self.section_style,
                )
            )
        ...
        for workout in week.get("daily_workouts", []):
            workout_data.append(
                [
                    Paragraph(
                        self._get_day_name(workout["day"]), self.table_cell_style
                    ),
                    Paragraph(workout["type"].title(), self.table_cell_style),
                    Paragraph(
                        f"{workout.get('distance', 0):.1f} km"
                        if workout.get("distance", 0) > 0
                        else "-",
                        self.table_cell_style,
                    ),
                    Paragraph(
                        workout.get("intensity", "-").title(), self.table_cell_style
                    ),
        ...
        header_styles[0] = ParagraphStyle(
            "DayHeader", parent=self.table_header_style, alignment=TA_CENTER
        )
        for i in range(len(workout_data[0])):
            workout_data[0][i] = Paragraph(workout_data[0][i], header_styles[i])

        workout_table = Table(
            workout_data, colWidths=[2.5 * cm, 2 * cm, 1.5 * cm, 1.5 * cm, 5 * cm]
        )
        ...
        if week.get("strength_training"):
            ...
            for st in week["strength_training"]:
                focus = (
                    (st.get("focus") or st.get("type", "")).replace("_", " ").title()
                )
                level = st.get("level", "")
    ...
    def _add_workout_steps_block(self, story: List, workouts: List[Dict[str, Any]]):
        ...
        stepped = [
            w
            for w in workouts
            if w.get("steps") and w.get("type") not in ("rest", "recovery")
        ]
        if not stepped:
            return
        ...
        for workout in stepped:
            ...
            story.append(
                Paragraph(f"▸ {day_name} — {w_type} · Session blocks", heading_style)
            )
    ...
    def _add_pace_improvement_summary(self, story: List, training_plan: PlanExportDTO):
        ...
        improvement = (
            (training_plan.current_pace - training_plan.goal_pace)
            / training_plan.current_pace
            * 100
        )
    ...
    def _add_training_zones_page(self, story: List, training_plan: PlanExportDTO):
        ...
        zone_table = Table(
            table_data,
            colWidths=[1.2 * cm, 2 * cm, 2 * cm, 3.5 * cm, 2.5 * cm, 5.3 * cm],
        )
        style_cmds = [
            ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor("#667eea")),
            ("TEXTCOLOR", (0, 0), (-1, 0), colors.whitesmoke),
    ...
    def _add_performance_weekly_plan(self, story: List, week: Dict[str, Any]):
        ...
        title = (
            f"Week {week['week']} - {week['total_km']:.1f} km | {phase.title()} Phase"
        )
        story.append(Paragraph(title, self.section_style))
        if phase_desc:
        ...
        for workout in week.get("daily_workouts", []):
            ...
            header_text = (
                f"<b>{day_name}</b> | {w_type} | {dist:.1f} km | {target_pace}"
            )
            ...
            if segments:
                ...
                seg_table = Table(
                    seg_data, colWidths=[5.5 * cm, 2.5 * cm, 3.5 * cm, 2.5 * cm]
                )
                seg_table.setStyle(
                    TableStyle(
                        [
