    ...
    for week in plan['weekly_plans']:
        ...
        for w in workouts:
            if w.get('quality', False) and long_dist > 0:
                ...
                if ratio > 1.1:
                    issues.append(f"W{week['week']}: {w['type']}={w['distance']:.1f}km > long={long_dist:.1f}km ({ratio:.2f}x)")

            if w['type'] == 'easy' and w['distance'] > 0:
                shortest_easy = min(shortest_easy, w['distance'])
                if w['distance'] < 2.0:
                    issues.append(f"W{week['week']}: easy={w['distance']:.1f}km (too short)")

        if not week.get('validation', {}).get('valid', True):
            issues.append(f"W{week['week']}: VALIDATION FAILED - {week['validation']['message']}")
    ...
    for week in weekly_plans:
        ...
        for w in workouts:
            if w['type'] in ('tempo', 'interval', 'hill') and long_dist > 0:
                ...
                worst_ratio = max(worst_ratio, ratio)
                if ratio > 1.1:
                    issues.append(f"W{week['week']}: {w['type']}={w.get('distance', 0):.1f}km > long={long_dist:.1f}km ({ratio:.2f}x)")
...
def main():
    ...
    for label, dist, km in perf_scenarios:
        ...
        print(f"{result['label']:<28} {result['worst_quality_ratio']:>9.2f}x {result['shortest_easy']:>9.1f} {status:>7} {result['total_km']:>9.1f}")
        if result['issues']:
    ...
    for label, km, dist in reg_scenarios:
        ...
        status = "OK" if not result['issues'] else f"{len(result['issues'])}"
        print(f"{result['label']:<28} {result['worst_quality_ratio']:>9.2f}x {result['shortest_easy']:>9.1f} {status:>7} {result['total_actual_km']:>9.1f}")
        if result['issues']:
