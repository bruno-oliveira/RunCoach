...
class TestACWRRiskPeakAdjustment:
    ...
    def test_high_risk_reduces_peak_by_15pct(self):
        ...
        assert 0.13 <= reduction <= 0.17, (
            f"Expected ~15% reduction, got {reduction:.1%}"
        )
    ...
    def test_very_high_risk_reduces_peak_by_25pct(self):
        ...
        peak_no_profile = get_peak_mileage(10.0, 25.0, 12)
        reduction = (peak_no_profile - peak) / peak_no_profile
        assert 0.23 <= reduction <= 0.27, (
            f"Expected ~25% reduction, got {reduction:.1%}"
        )

    def test_high_risk_10k_plan_lower_total_volume(
        ...
        assert total_high < total_normal, (
            "High-risk plan should have lower total volume"
        )

    def test_very_high_risk_marathon_plan_significantly_lower(
        self, plan_generator: TrainingPlanGenerator
    ):
        """A marathon plan with very_high ACWR risk should be notably lower volume."""
        profile_vh = _profile(
            acwr_risk="very_high", avg_weekly_km=40.0, longest_run_km=28.0
        )
        ...
        assert peak_vh < peak_normal * 0.85, (
            "Very-high-risk peak should be <85% of normal"
        )
...
class TestVolumeTrendRampAdjustment:
    ...
    def test_decreasing_trend_plan_has_gentler_ramp(
        self, plan_generator: TrainingPlanGenerator
    ):
        """Plan with decreasing trend should ramp more gently."""
        profile_dec = _profile(
            volume_trend="decreasing", avg_weekly_km=20.0, acwr_risk="low"
        )
        ...
        normal_non_rec = [
            w["total_km"] for w in plan_normal[:6] if not w["is_recovery"]
        ]
        for d, n in zip(dec_non_rec, normal_non_rec):
            assert d <= n + 1.0, (
                f"Decreasing trend week {d} should not exceed normal {n}"
            )
...
class TestLongestRunCeiling:
    ...
    def test_longest_run_does_not_block_marathon_build(
        self, plan_generator: TrainingPlanGenerator
    ):
    ...
    def test_high_longest_run_does_not_constrain(
        self, plan_generator: TrainingPlanGenerator
    ):
        ...
        wk1_normal = next(
            (
                w["distance"]
                for w in plan_normal[0]["daily_workouts"]
                if w["type"] == "long"
            ),
            0,
        )
...
class TestPaceZoneWorkoutMix:
    ...
    def test_balanced_profile_preserves_standard_mix(self):
        ...
        assert quality == 1, (
            f"Balanced profile should preserve standard mix, got {quality}"
        )
...
class TestWorkoutTypeGaps:
    ...
    def test_no_tempo_uses_tempo_in_base(self):
        ...
        assert dist.get("tempo", 0) >= 1, "No-tempo runner should get tempo in base"
...
class TestFullPlanWithProfile:
    ...
    def test_high_risk_profile_produces_conservative_plan(
        self, plan_generator: TrainingPlanGenerator
    ):
        """High ACWR risk profile should produce a more conservative plan."""
        profile_high = _profile(
            acwr_risk="high", avg_weekly_km=30.0, longest_run_km=15.0
        )
        plan_high = plan_generator.generate_plan(
            current_km=30.0,
    ...
    def test_decreasing_trend_profile_ramps_gently(
        self, plan_generator: TrainingPlanGenerator
    ):
        """Decreasing trend profile should ramp more gently."""
        profile_dec = _profile(
            volume_trend="decreasing", avg_weekly_km=20.0, longest_run_km=12.0
        )
        plan = plan_generator.generate_plan(
            current_km=20.0,
            target_distance=10,
    ...
    def test_improving_runner_gets_aggressive_ramp(
        self, plan_generator: TrainingPlanGenerator
    ):
    ...
    def test_profile_with_all_fields_produces_coherent_plan(
        self, plan_generator: TrainingPlanGenerator
    ):
        ...
        for week in plan:
            ...
            assert len(long_runs) == 1, (
                f"Week {week['week']} should have exactly 1 long run"
            )

    def test_profile_none_same_as_no_profile(
        self, plan_generator: TrainingPlanGenerator
    ):
    ...
    def test_profile_vdot_used_when_not_explicitly_provided(
        self, plan_generator: TrainingPlanGenerator
    ):
    ...
    def test_profile_avg_weekly_km_used_when_higher(
        self, plan_generator: TrainingPlanGenerator
    ):
    ...
    def test_profile_avg_weekly_km_ignored_when_lower(
        self, plan_generator: TrainingPlanGenerator
    ):
...
class TestProfileEdgeCases:
    ...
    def test_profile_with_empty_workout_counts(
        self, plan_generator: TrainingPlanGenerator
    ):
