...
def _all_import_targets(path: pathlib.Path):
    """Yield every imported module in a file, at any nesting depth (top level,
...
def test_all_cross_context_access_goes_through_application_ports():
    """No bounded context imports a sibling context *at all* — not even via a
    deferred/TYPE_CHECKING import.

    Cross-context collaboration is funnelled through the single application
    seam ``app.application.ports`` (which resolves the sibling lazily), so the
    contexts never name each other's modules directly. This is stricter than
    the module-level guard above and keeps the cross-context wiring auditable
    in one place.
    """
    names = _context_names()
    violations: list[str] = []
    ...
    assert not violations, (
        "Bounded contexts must reach a sibling only through "
        "app.application.ports, never by importing the sibling directly:\n  "
        + "\n  ".join(sorted(violations))
    )
